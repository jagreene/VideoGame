...
class Model:
    ...
    def __init__(self, screen):
        ...
        self.screen = screen
        self.buttons = Group(
            Button(700, 450, "1P", "Pictures/1P.png"), Button(900, 450, "2P", "Pictures/2P.png"))
        self.time = 90
        self.endTime = 5
        ...
        for rows in xrange(0, (size[1] / 50)):
            for columns in xrange(0, (size[0] / 50)):
                if rows == (size[1] / 50) - 1 or rows == 0 or columns == (size[0] / 50) - 1 or columns == 0:
                    ...
                    map.update(
                        {(rows, columns): random.choice(map_options[:1])})
    ...
    def drawMap(self):
        ...
        for position, contain in self.map.items():
            if contain is "block":
                ...
                self.Coins.add(
                    Coins(position[1] * 50 + 10, position[0] * 50 + 10))
...
class View():
    ...
    def displayStartScreen(self):
        ...
        Title = myfont.render(
            "THE WORLD ENDS WITH COINS", 1, random.choice(all_color))
    ...
    def displayEndingScreen(self):
        ...
        Title = myfont.render("Player %s Wins" %
                              (self.model.winner), 1, random.choice(all_color))
    ...
    def update(self):
        ...
        p1Score = myfont.render(
            "Player 1 Score:" + str(self.model.player1.score), 1, whiteColor)
        self.screen.blit(p1Score, (100, 15))
        if self.model.playernum == 2:
            p2Score = myfont.render(
                "Player 2 Score:" + str(self.model.player2.score), 1, whiteColor)
            self.screen.blit(p2Score, (1200, 15))
        timerLimit = myfont.render(
            "Time Limit:" + str(self.model.time), 1, whiteColor)
...
class Coins(Sprite):
    ...
    def __init__(self, x, y, pictures=["Pictures/Coin1.png", "Pictures/Coin2.png", "Pictures/Coin3.png", "Pictures/Coin4.png", "Pictures/Coin5.png", "Pictures/Coin6.png", "Pictures/Coin7.png", "Pictures/Coin8.png"]):
        ...
        # not necessary right now, but for later
...
# Creates font object for displaying text to screen
myfont = pygame.font.SysFont("Liberation Mono", 35, True, False)
# Sets the window of pygame
screen = pygame.display.set_mode((1600, 900), pygame.FULLSCREEN)
# grabs the current displays information to get size of screen
info = pygame.display.Info()
# stores size of screen, allows us to divide screen to create rows and columns
...
while running:  # enters game loop
    ...
    # checks collisions for every block, coin, and player
    for player in m.players.sprites():
    ...
    # decrements the timer and handles ending game when timer equals zero
